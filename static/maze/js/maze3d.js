(function() {
   ...
   function pad(val) {
    ...
    function startTimer(){
      start = new Date();
      console.log(start);
      socket.emit('maze', {final_score:finalScore, status: "start"});
    }
    ...
    function endTimer() {
      ...
      console.log(seconds + " seconds"); //take this for model

      if(pc.length != 0){
        sum = pc.reduce(add);
      }

      finalScore = ((1/(seconds+sum) + 0.5)*100);
    ...
    function initializeScene() {
        ...
        var ground = new THREE.Mesh(floorGeometry, new THREE.MeshPhongMaterial({
            map: loader.load("/static/maze/images/textures/ground_diffuse.jpg"),
        }));
        ...
        var topMesh = new THREE.Mesh(floorGeometry, new THREE.MeshPhongMaterial({
            map: loader.load("/static/maze/images/textures/roof_diffuse.jpg")
        }));
        ...
        var wallMaterial = new THREE.MeshPhongMaterial({
            map: loader.load("/static/maze/images/textures/wall_diffuse.jpg")
        });
    ...
    function moveCamera(direction, delta) {
        ...
        if (map[newTy][newTx] != 1 && !isNaN(map[newTy][newTx])) {
            ...
            bumps = bumps + 1;
            pc[bumps-1] = 0.5*Math.log(totalSeconds) + 0.6;
            document.getElementById("bumps").innerHTML = bumps;
            collideFlag = true;
          }
        } else if (map[newTy][newTx] == "A") {
            // Game is over
            endTimer();
            console.log({time: seconds,bump: bumps})

            socket.emit('maze', {final_score:finalScore,status: "finish"});
    ...
    function endScreen() {
      ...
      alert("Good job, The game is over\n\nThanks you for playing!");
        // if (levelHelper.isFinished == true) {
        //     alert("Good job, The game is over\n\nThanks you for playing!");
    ...
    function loadLevel(level) {
        ...
        ajax.open("GET", "/static/maze/maps/maze3d-" + level + ".json", true);
